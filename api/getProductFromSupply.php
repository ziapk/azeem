...
$supplierId = !empty($_GET['supplierId']) ? $_GET['supplierId'] : false;
if(!empty($supplierId)) {
    if(!empty($_SESSION['shopInfo'])) {
        $search = $products->getSupplyProductsPagination($ownerId, $supplierId, ['page' => (int)$page, 'perPage' => (int)$perPage, 'search' => $search, 'searchBy' => $searchBy, 'courceId' => $courceId, 'group' => $group, 'board' => $board, 'full_name' => $full_name, 'author' => $author, 'sortByField' => $sortByField, 'sortByOrder' => $sortByOrder, 'pin' => $pin, 'publisher_id' => $publisher_id, 'correction' => $correction], $shopId);
    };
}
