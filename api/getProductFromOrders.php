...
$customerId = !empty($_GET['customerId']) ? $_GET['customerId'] : false;
if(!empty($customerId)) {
    if(!empty($_SESSION['shopInfo'])) {
        $search = $products->getOrderProductsPagination($ownerId, $customerId, ['page' => (int)$page, 'perPage' => (int)$perPage, 'search' => $search, 'searchBy' => $searchBy, 'courceId' => $courceId, 'group' => $group, 'board' => $board, 'full_name' => $full_name, 'author' => $author, 'sortByField' => $sortByField, 'sortByOrder' => $sortByOrder, 'pin' => $pin, 'publisher_id' => $publisher_id, 'correction' => $correction], $shopId);
    };
}
