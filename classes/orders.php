...
class Orders extends Connection
{
    ...
    public function getProductSales($params = [])
    {
        ...
        try {
            ...
            $stmt = "SELECT o.id, o.order_custom_id, oi.product_id, o.order_date, oi.price AS price, oi.discount, oi.quantity AS quantity, p.full_name, c.full_name as customerName  FROM `{$this->table}` AS o LEFT JOIN `{$this->table_sub}` AS oi ON oi.order_id=o.id LEFT JOIN `{$this->table_pro}` AS p on p.id=oi.product_id LEFT JOIN customers AS c ON c.id = o.customer_id WHERE o.shopId=:shopId " . $toCondition . " and o.flag = 1 ORDER BY o.order_date desc";
            $prepare = $dbh->prepare($stmt);
            $prepare->bindParam(':shopId', $shopId, PDO::PARAM_STR);
            $prepare->execute();
            $result = $prepare->fetchAll(PDO::FETCH_ASSOC);
