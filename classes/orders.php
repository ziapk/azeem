...
class Orders extends Connection
{
    ...
    public function ordersReportSummery($shopId, $date, $to, $publisher_id = null, $product_id = [], $report = '')
    {
        ...
        try {
            ...
            $stmt = "SELECT count(o.id) AS total, ROUND(SUM(o.price * o.quantity), 2) AS gross, ROUND(SUM(o.discount), 2) AS dist, ROUND(SUM(o.paid_amount), 2) AS paid, ROUND(SUM((o.`price` * o.quantity) - o.`discount` - o.`paid_amount`), 2) AS balance FROM `{$this->table}` AS o " . $join . " WHERE o.shopId=:shopId " . $toCondition . ' and o.flag = 1 ORDER BY o.id desc';
            $prepare = $dbh->prepare($stmt);
            $prepare->bindParam(':shopId', $shopId, PDO::PARAM_STR);
            $prepare->execute();
            $result = $prepare->fetch(PDO::FETCH_ASSOC);
