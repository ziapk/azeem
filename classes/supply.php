    ...
    public function ordersReport($shopId, $date, $to, $ids = [], $publisher_id = null, $account_id = null)
    {
        ...
        try {
            ...
            if (!empty($account_id) || !empty($publisher_id) || !empty($ids)) {
                ...
                $stmt = "SELECT sub.*, sub.quantity AS tquantity, round(sub.pprice, 2) as pprice, round(sub.price, 2) as price, round(sub.pprice * sub.quantity, 2) AS tpprice, round(sub.price * sub.quantity, 2) AS tprice, round((sub.price - sub.pprice) * sub.quantity, 2) AS tdiscount, round(sub.price - sub.pprice, 2) AS discount, sub.supply_id as order_custom_id, o.supply_date as order_date, p.full_name as productName, c.full_name, s.name FROM `{$this->table_sub}` AS sub left join `{$this->table_pro}` as p on p.id = sub.product_id left join `{$this->table}` as o on sub.supply_id = o.id LEFT JOIN customers AS c ON c.id = o.supplier_id and o.supplier_type = 2 LEFT JOIN `{$this->table_suppliers}` AS s ON s.id = o.supplier_id and o.supplier_type = 1 WHERE o.shopId=:shopId " . $toCondition . ' and o.flag = 1 ORDER BY o.supply_date asc, sub.quantity desc';
            } else {
                $stmt = "SELECT sub.*, sub.quantity AS tquantity, round(sub.pprice, 2) as pprice, round(sub.price, 2) as price, round(sub.pprice * sub.quantity, 2) AS tpprice, round(sub.price * sub.quantity, 2) AS tprice, round((sub.price - sub.pprice) * sub.quantity, 2) AS tdiscount, round(sub.price - sub.pprice, 2) AS discount, sub.supply_id as order_custom_id, o.supply_date as order_date, p.full_name as productName, c.full_name, s.name FROM `{$this->table_sub}` AS sub left join `{$this->table_pro}` as p on p.id = sub.product_id left join `{$this->table}` as o on sub.supply_id = o.id LEFT JOIN customers AS c ON c.id = o.supplier_id and o.supplier_type = 2 LEFT JOIN `{$this->table_suppliers}` AS s ON s.id = o.supplier_id and o.supplier_type = 1 WHERE o.shopId=:shopId " . $toCondition . ' and o.flag = 1 ORDER BY o.supply_date asc, sub.quantity desc';
            }
