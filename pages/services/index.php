...
echo mainHeader(['page' => 'services']);
?>

<div class="container" ng-controller="categoryController">
    <a href="javascript:void(0)" style="margin-right: 10px" ng-click="addCategory()" class="btn btn-primary btn-xs pull-right">Add Serivce</a>
    <h4>All Services</h4>
    ...
    <script type="text/ng-template" id="addCategory.html">
        ...
        <div class="modal-header">
            <h3 class="modal-title" id="modal-title">Add Service</h3>
        </div>
        <div class="modal-body" id="modal-body">
            <div class="form-group">
                <label for="sfull_name">Name</label>
                <input id="sfull_name" type="text" ng-model="form.full_name" name="full_name" class="form-control" placeholder="Serivce Name">
            </div>
        </div>
    ...
    <script>
        app.controller('categoryController', function($scope, $http, $httpParamSerializerJQLike, $filter, $window, $document, $uibModal, $log) {
            ...
            $scope.getCategories = (page) => {
                $scope.loading = true;
                $http.get($scope.siteUrl + "api/getServices.php", {
                        params: {
                            page: page || 1,
