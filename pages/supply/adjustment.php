    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <th width="200">
                    {{supplier.opening_balance - subTotal > 0 ? supplier.opening_balance - subTotal : 0}}
                </th>
            </tr>
            <tr ng-if="grandTotal">
    ...
    app.controller('reportController', function($scope, $http, $window, $httpParamSerializerJQLike) {
        ...
        $scope.selectSupplier = function(p) {
            $http.get("<?php echo SITE_URL ?>api/getOpeningBalance.php", {
                ...
                $scope.supplier = {
                    ...p,
                    opening_balance: ((parseFloat(res.data.opening_balance || 0) + parseFloat(res.data.creditAmount || 0)) - parseFloat(res.data.debitAmount || 0))
                };
        ...
        $scope.searchProduct = function(term) {
            const filteredArray = window.mainList.records.filter(r => r.is_active == 1).filter(r => r.id == term || r.code == term || r.searchString.split('|').pop()?.toLowerCase().includes(term?.toLowerCase()));
        ...
        $scope.checkout = function() {
            ...
            $http.post("<?php echo SITE_URL ?>api/placeSupplierReturn.php", $httpParamSerializerJQLike($scope.form), {
                    headers: {
