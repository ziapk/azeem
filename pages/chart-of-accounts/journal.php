...
if(!empty($_POST)) {
  ...
  $demands = $doubleEntryObj->getJournals($arr);
  $acd_ids = [];
  $grouping = [];
...
<div class="container" ng-controller="accountingController">
	<div class="content-section">
        ...
        <table class="table table-sm table-func table-hover">
            ...
            <tbody>			
            <?php $count = 1; foreach ($grouping as $id => $rows){  ?>
                  <tr>
                      <td>
                        <?php echo $count; ?>
                      </td>
                ...
                <?php foreach ($rows as $key => $product){ 
                  ...
                  ?>
                    <tr>
                        ...
                        <td><?php if($product['entry_type'] == ($arr['account_id'] ? 'D' : 'C')) { echo $product['amount']; } ?></td>
                        <td></td>
                    </tr>				
                <?php }} $count++; } ?>
            </tbody>
...
app.controller('accountingController', function($scope, $http, $window){
    ...
    $scope.searchGroup = function (term) {
        return $http.get("./getAccountLeafs.php", {params: {term}})
