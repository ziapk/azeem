...
app.controller('coaController', function ($scope, $http, $httpParamSerializerJQLike) {
  ...
  $('#newAccountForm').validate({
    ...
    submitHandler: function(form) {
      ...
      $http.post(url, params, {headers: {'Content-Type': 'application/x-www-form-urlencoded'} })
      .then(res => {
          form.reset();
  ...
  $scope.deleteAccount = (obj) => {
    ...
    if(confirm('Are you sure you want to delete Account: '+ obj.title+' ('+obj.code+')')) {
      $http.post(url, $httpParamSerializerJQLike({id: obj.id}), {headers: {'Content-Type': 'application/x-www-form-urlencoded'} })
      .then(res => {
          window.location.href = window.location.href;
      });
    }
