...
<div class="container" ng-controller="productsController">
    <div class="form-group">
        <div class="row">
            ...
            <button type="button" class="btn btn-primary" ng-click="updateProducts()">Update Products</button>
...
app.controller('productsController', function($scope, $http, $httpParamSerializerJQLike, $filter, $window, toaster) {
    ...
    $scope.getProducts = (page) => {
        $scope.loading = true;
        $http.get("<?php echo SITE_URL?>api/getProducts.php", {params: {page: page || 1, perPage: $scope.data.perPage, search: $scope.search, full_name: $scope.full_name, group: $scope.group, author: $scope.author, board: $scope.board, searchBy: $scope.searchBy, courceId: $scope.courceId, publisher_id: $scope.publisher_id, correction: true}})
    ...
    $scope.updateProducts = (page) => {
        $scope.loading = true;
        $http.get("<?php echo SITE_URL?>api/getProducts.php", {params: {page: page || 1, perPage: $scope.data.perPage, search: $scope.search, full_name: $scope.full_name, group: $scope.group, author: $scope.author, board: $scope.board, searchBy: $scope.searchBy, courceId: $scope.courceId, publisher_id: $scope.publisher_id, correction: true, selectedPublisherId: $scope.form.publisher_id }})
        .then(function(response) {
            $scope.loading = false;
            if(response.status === 200) {
                $scope.data = response.data;
