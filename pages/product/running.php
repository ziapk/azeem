    ...
    <ul class="feature-products">
        <li ng-repeat="li in list" class="feature-product">
            <div class="product-image">
                ...
                <a href="javascript:void(0)" ng-click="addBookmark(li)" class="btn-bookmark" uib-tooltip="Remove from running"><img width="20" height="20" src="<?php echo SITE_URL; ?>assets/img/svg/delete.svg" alt="" /></a>
...
app.controller('productsController', function($scope, $http, $httpParamSerializerJQLike, $filter, $window, toaster) {
    ...
    $scope.getProducts = (page) => {
        $scope.loading = true;
        $http.get("<?php echo SITE_URL?>api/getProducts.php", {params: {page: page || 1, perPage: $scope.data.perPage, search: $scope.search, full_name: $scope.full_name, group: $scope.group, author: $scope.author, board: $scope.board, searchBy: $scope.searchBy, courceId: $scope.courceId, bookmark: 1}})
    ...
    $scope.addBookmark = function (item) {
        ...
        .then(function(response) {
            if(response.status === 200) {
                toaster.success({body: 'Successfully Removed!'});
                $scope.getProducts();
            }
        })
    }
