    ...
    <script>
        app.controller('programController', function($scope, $http, $httpParamSerializerJQLike, $filter, $window, $document, $uibModal, $log) {
            ...
            $scope.getPrograms = (page) => {
                $scope.loading = true;
                $http.get($scope.siteUrl + "api/getPrograms.php", {
                        params: {
                            page: page || 1,
                            search: $scope.search,
            ...
            $scope.addProgram = function(size, parentSelector) {
                ...
                $uibModal.open({
                    ...
                    $http.post('create.php', $httpParamSerializerJQLike(selectedItem), {
                        headers: {
            ...
            $scope.assignBooks = function(item) {
                $uibModal.open({
                    ...
                    $http.post($scope.siteUrl + 'api/assignBooks.php', $httpParamSerializerJQLike(response), {
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        }
                    }).then(function() {
        ...
        app.controller('ModalInstanceCtrl', function($scope, $uibModalInstance, $http) {
            ...
            $scope.ok = function() {
                $uibModalInstance.close({
                    degree: $scope.form.degree.title || $scope.form.degree,
                    program: $scope.form.program.title || $scope.form.program,
                    class: $scope.form.class.title || $scope.form.class
                });
            };
            ...
            $scope.searchDegree = function(term) {
                return $http.get("<?php echo SITE_URL ?>api/getProgramsFields.php", {
                        params: {
                            term,
                            view: 'degree'
                        }
        ...
        app.controller('AssignBooksModalInstanceCtrl', function($scope, $http, $uibModalInstance, parentData) {
            ...
            $scope.getBooks = () => {
                return $http.get("<?php echo SITE_URL ?>api/getBooks.php", {
                        params: {
                            id: parentData.id
                        }
                    })
