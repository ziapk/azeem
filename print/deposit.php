...
foreach ($recevingEntry as $row) {
    if ($row['entry_type'] == 'D') {
        if (in_array($row['parent_id'], [$storeAccounts['locker']])) {
            $userEntry = $row;
            $blc = $de->getOpeningBalance($row['account_id'], 'c');
        }
    } else {
        if (in_array($row['parent_id'], [$storeAccounts['locker']])) {
            $configs['title'] = 'Locker Entry';
            $configs['label'] = 'Locker\'s Name';
            $configs['sign_label'] = 'Owner\'s Sign';
            $userEntry = $row;
            $userEntry['debitAmount'] = $row['amount'];
                    ...
                    <table class="recipt-table" width="100%" cellpadding="0" cellspacing="0" cellspading="0" cellspacing="0">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="text-right" style="font-size: 20px;"><?php echo number_format(($userEntry['debitAmount'])); ?></td>
                            </tr>
                            <tr class="no-border">
                                <th style="border: 0;" valign="middle" colspan="4" class="text-left">
                                    Net in words: <?php echo convertNumberToWord($userEntry['debitAmount']); ?>
                                </th>
                            </tr>
                            <tr>
